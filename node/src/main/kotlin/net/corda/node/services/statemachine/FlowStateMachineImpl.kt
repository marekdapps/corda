...
class FlowStateMachineImpl<R>(override val id: StateMachineRunId,
    ...
    private inline fun <reified M : ExistingSessionMessage> receiveInternal(
        ...
        return suspendAndExpectReceive(ReceiveOnly(session, M::class.java), userReceiveType)
    }
    ...
    private inline fun <reified M : ExistingSessionMessage> sendAndReceiveInternal(
            ...
            userReceiveType: Class<*>?): ReceivedSessionMessage<M> {
        return suspendAndExpectReceive(SendAndReceive(session, message, M::class.java), userReceiveType)
    }
    ...
    private fun <M : ExistingSessionMessage> suspendAndExpectReceive(receiveRequest: ReceiveRequest<M>, userReceiveType: Class<*>?): ReceivedSessionMessage<M> {
        ...
        if (receiveRequest.receiveType.isInstance(receivedMessage.message)) {
            ...
            if (receivedMessage.message is ErrorSessionEnd) {
                ...
                val expectedType = userReceiveType?.name ?: receiveRequest.receiveType.simpleName
