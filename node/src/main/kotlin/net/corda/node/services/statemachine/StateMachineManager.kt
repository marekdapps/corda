...
class StateMachineManager(val serviceHub: ServiceHubInternal,
    ...
    private fun onExistingSessionMessage(message: ExistingSessionMessage, sender: Party) {
        ...
        if (session != null) {
            ...
            val resumeFiber = (session.fiber.waitingForResponse as? ReceiveRequest<*>)?.session === session ||
                    session.fiber.waitingForResponse is WaitForLedgerCommit && message is ErrorSessionEnd
            if (resumeFiber) {
                // It's important that we reset the flat here and not after the fiber's resumed, in case we receive
                // another message before then.
